...
?>
                        ...
                        <script>
                            ...
                            function hidetamu() {
                                ...
                                $.get("<?= base_url(); ?>/tamu", {
                                        ...
                                        transaction_tamu: transaction_tamu
                                    })
                                    .done(function(data) {
                                        // alert(data);
                                    });
                    ...
                    <tbody>
                        ...
                        if ($usr->transactiond_percent > 0) {
                            ...
                            if ($usr->product_start == null) {
                                $usr->product_start = "0000-00-00 00:00:00";
                            }
                            if ($usr->user_id == null) {
                                $usr->user_id = 0;
                            }
                        ...
                        ?>
                            <tr class="">
                                ...
                                <td class="text-small0">
                                    ...
                                    if ($usr->product_durasi > 0 && $usr->product_lanjutan == 0) {
                                        if (date("Y-m-d H:i:s") > $usr->product_start && date("Y-m-d H:i:s") < $usr->product_bend) {
                                            $statusroom = "success";
                                        } elseif (date("Y-m-d H:i:s") >= $usr->product_bend && date("Y-m-d H:i:s") <= $usr->product_end) {
                                            $statusroom = "warning";
                                        } else {
                                            $statusroom = "danger";
                                        }
                                    ...
                                    <div class=" linekecil ">
                                        ...
                                        <?php } ?>
                                        <span class="linekecil">
                                            <?= $usr->product_name; ?>
                                            <?php if ($usr->category_name == "PAKET") { ?>
                                                (<span class="badge badge-primary"><?= $usr->product_roomqty; ?> Room</span>)
                                            <?php } ?>
                                        </span>
                                    </div>
                                    <?php if ($usr->category_name == "PAKET") {
                                        $productb = $this->db->table("productb")
                                            ->where("productb.product_id", $usr->product_id)
                                            ->join("product", "product.product_id=productb.productb_bundle", "left")
                                            ->get();
                                        foreach ($productb->getResult() as $productb) { ?>
                                            <button class="btn btn-xs btn-outline-danger" style="color:blue;"><?= $productb->product_name; ?></button>
                                        <?php } ?>
                                    <?php } ?>
                                    ...
                                    <?php } ?>
                                    <?php if ($usr->product_durasi > 0 && $usr->product_lanjutan == 0) { ?>
                                        <div class=" badge-success linekecil fdnota mb1 p5 mt4"><?= $usr->product_start; ?></div>
                                        <div class=" badge-warning linekecil fdnota mb1 p5"><?= $usr->product_bend; ?></div>
                                        <div class=" badge-danger linekecil fdnota mb1 p5"><?= $usr->product_end; ?></div>
                                    <?php } ?>
                                </td>
                                <td class="text-small0">
                                    <?php
                                ...
                                <td class="text-small0">
                                    <div style="font-size:10px; color:red; margin-bottom:0px;"><?= $kdis; ?></div><?= number_format($diskon, 0, ",", "."); ?>
                                </td>
    ...
    public function listproductgambar()
    {
        ...
        $builder->where("product.product_onoff", 1);
        $builder->orderBy("category.category_urutan", "ASC");
        $builder->orderBy("product.product_urutan", "ASC");
        ...
        foreach ($product->getResult() as $product) {
            if ($product->product_durasi > 0 && $product->product_lanjutan == 0) {
                $start = 1;
            } else {
                $start = 0;
            }
            if ($product->position_id > 0) {
                $therapist = 1;
            } else {
                $therapist = 0;
            }
        ?>
            ...
            if (
                ...
                if ($product->product_status > 0) {
                    $insertnota = "toast('Info', 'Room sedang Rusak atau Dibersihkan!')";
                    $disabled = "disabled";
                } else
                ...
                if ($product->transaction_status == 2 && $product->category_unique == 1) {
                    $disabled = "disabled";
                    $insertnota = "toast('Info Status', 'Tidak Tersedia!')";
                }
            ...
            <tbody>
                ...
                ?>
                    ...
                    if (
                        ...
                        if ($usr->transaction_status == 2 && $usr->category_unique == 1) {
                            $disabled = "disabled";
                            $insertnota = "toast('Info Status', 'Tidak Tersedia!')";
                        }
                    } else {
